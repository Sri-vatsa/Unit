...
if __name__ == '__main__':
    ...
    parser.add_argument('--image-path', type=str, default='../../measurement_dataset_small')
    ...
    images = [cv2.imread(file) for file in glob.glob(args.image_path + '/*.jpg')]
    print(len(images))
    counter = 0
    all_measurements_mapping = {}
    for image in images:
        ...
        cv2.imwrite("images/processed/processed_image%s.jpeg" % counter, image)
        counter = counter + 1
        all_measurements_mapping[counter] = body_coordinates
    ...
    # Convert detected body measurements to csv

    toCSV = []

    # Preprocess values in results
    for img_id in all_measurements_mapping:
        for dictionary in all_measurements_mapping[img_id]:
            dictionary["img_id"] = img_id
            toCSV.append(dictionary)
